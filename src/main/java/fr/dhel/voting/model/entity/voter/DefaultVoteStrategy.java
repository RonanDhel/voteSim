...
public class DefaultVoteStrategy implements VoteStrategy {
	...
	Pair<Candidate, Double> getCandidateWithScore(
			final double utility, final Candidate candidate, final int minRange, final int maxRange) {
	...
	private List<Pair<Candidate, Double>> computeResultsNormalCase(
		...
		return candidatesByUtility
				.stream()
				.map(c -> getCandidateWithScore(
						voter.utility(c).divide(maximalUtility, RoundingMode.HALF_EVEN)
								.doubleValue(), c, minRange, maxRange)).collect(toList());
	}
	...
	public List<Pair<Candidate, Double>> getListOfCandidateWithScore(
		...
		if (maximalUtility.compareTo(voter.utility(worstCandidate)) == 0) {
			...
			return candidateSet.stream()
					.map(c -> getCandidateWithScore(0.5, c, minRange, maxRange)).collect(toList());
		}

		return unmodifiableList(computeResultsNormalCase(voter, candidatesByUtility, maximalUtility, minRange,
				maxRange));
	}
}
