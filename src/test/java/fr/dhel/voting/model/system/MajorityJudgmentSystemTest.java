...
public class MajorityJudgmentSystemTest {
	...
	@Before
	public void setup() {
		when(ballotWithPerfectScoreForSithAndLowScoreForYoda.computeResults()).thenReturn(
				Arrays.asList(Pair.of(anakinSkywalker, 4.0), Pair.of(hanSolo, 3.0),
						Pair.of(obiWan, 3.0), Pair.of(yoda, 2.0), Pair.of(palpatine, 4.0)));

		when(ballotWithMainlyHighValue.computeResults()).thenReturn(
				Arrays.asList(Pair.of(anakinSkywalker, 3.0), Pair.of(hanSolo, 3.0),
						Pair.of(obiWan, 3.0), Pair.of(yoda, 4.0), Pair.of(palpatine, 4.0)));

		when(ballotWithOnlyLowValueAndNotPointForPalpatine.computeResults()).thenReturn(
				Arrays.asList(Pair.of(anakinSkywalker, 2.0), Pair.of(hanSolo, 1.0),
						Pair.of(obiWan, 2.0), Pair.of(yoda, 2.0), Pair.of(palpatine, 0.0)));

		when(ballotMainlyForPalpatineAndHanSolo.computeResults()).thenReturn(
				Arrays.asList(Pair.of(anakinSkywalker, 1.0), Pair.of(hanSolo, 3.0),
						Pair.of(obiWan, 2.0), Pair.of(yoda, 2.0), Pair.of(palpatine, 4.0)));

		when(ballotForJedi.computeResults()).thenReturn(
				Arrays.asList(Pair.of(anakinSkywalker, 3.0), Pair.of(hanSolo, 1.0),
						Pair.of(obiWan, 2.0), Pair.of(yoda, 4.0), Pair.of(palpatine, 0.0)));

		when(ballotWithMediumValue.computeResults()).thenReturn(
				Arrays.asList(Pair.of(anakinSkywalker, 1.0), Pair.of(hanSolo, 2.0),
						Pair.of(obiWan, 2.0), Pair.of(yoda, 2.0), Pair.of(palpatine, 2.0)));

		when(ballotWithOnlyLowValueAndNotPointForHanSolo.computeResults()).thenReturn(
				Arrays.asList(Pair.of(anakinSkywalker, 1.0), Pair.of(hanSolo, 0.0),
						Pair.of(obiWan, 2.0), Pair.of(yoda, 2.0), Pair.of(palpatine, 2.0)));
	...
	public void shortName_ShouldReturnTheName() {
		assertThat(new MajorityJudgmentSystem().shortName(), is(equalTo("MJ")));
	}

	@Test
	public void createBallot_ShouldReturnABallotBuilder() {
		assertNotNull(new MajorityJudgmentSystem().createBallot(getCandidates()));
	}
	...
	public void countVotes_ShouldElectTheCandidateWithBestMedianButNotNecessarilyTheHighestAverage() {
		...
		assertThat(mjs.countVotes(votes, getCandidates()).getElectedCandidate(),
				is(equalTo(palpatine)));
	}
