...
public class FourRoundPluralitySystemTest {
    ...
    public void shortName_ShouldReturnTheName() {
        ...
        assertThat(ps.shortName()).isEqualTo("PL4");
    }

    @Test
    public void isPluralityType_ShouldReturnTrue() {
        assertThat(new FourRoundPluralitySystem().isPluralityType()).isTrue();
    }
    ...
    public void createBallot_ShouldReturnABallotBuilder() {
        assertThat(new FourRoundPluralitySystem().createBallot(getCandidates())).isNotNull();
    }
    ...
    public void countVotes_ShouldElectTheOneWithAnAbsoluteMajority() {
        ...
        assertThat(ps.countVotes(votes, getCandidates()).getElectedCandidate()).isEqualTo(luffy);
    }
    ...
    public void countVotes_ShouldRemoveAllWeakCandidateIfNoOneHaveAnAbsoluteMajority() {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            Ballot b = mock(Ballot.class);
            when(b.computeResults()).thenReturn(Collections.singletonList(Pair.of(sanji, 1.0)));
            votes.add(b);
        }
        for (int i = 0; i < 7; i++) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            Ballot b = mock(Ballot.class);
            when(b.computeResults()).thenReturn(Collections.singletonList(Pair.of(chopper, 1.0)));
            votes.add(b);
        }
        // 1 votes sur 40 soit 2.5%
        Ballot b = mock(Ballot.class);
        when(b.computeResults()).thenReturn(Collections.singletonList(Pair.of(robin, 1.0)));
        votes.add(b);
        // 1 votes sur 40 soit 2.5%
        Ballot b2 = mock(Ballot.class);
        when(b2.computeResults()).thenReturn(Collections.singletonList(Pair.of(franky, 1.0)));
        votes.add(b2);

        assertThat(ps.countVotes(votes, candidates).needNextRound()).isTrue();
        Set<Candidate> expected = new HashSet<>();
        expected.add(luffy);
        expected.add(nami);
        expected.add(sanji);
        expected.add(zorro);
        assertThat(ps.countVotes(votes, candidates).candidatesForNextRound()).isEqualTo(expected);
    }
    ...
    public void countVotes_ShouldSelectTopTwoOnThirdTurn() {
        ...
        assertThat(ps.countVotes(votes, getCandidates()).needNextRound()).isTrue();

        Set<Candidate> res = ps.countVotes(votes, getCandidates()).candidatesForNextRound();

        assertThat(res).hasSize(2);
        assertThat(res).contains(luffy, nami);
    }
    ...
    public void countVotes_ShouldSelectBest3IfTooManyWeakCandidate() {
        ...
        for (int i = 0; i < 40; i++) {
            ...
            if (i < 30) {
                ...
                when(b.computeResults()).thenReturn(Collections.singletonList(Pair.of(brook, 1.0)));
        ...
        assertThat(res).contains(luffy, sanji, brook);
    }
}
